...
if __name__ == "__main__":
    ...
    # check whether the robot is reset, and the collision is False
    ...
    # check whether the robot has started to move
    ...
    while (compute_distance(goal_coor, curr_coor) > 1) and (not collided) and (curr_time - start_time < time_limit):
        curr_time = rospy.get_time()
    ...
    if collided:
        status = "collided"
    elif curr_time - start_time >= time_limit:
        status = "timeout"
    else:
        status = "succeeded"
        success = True
